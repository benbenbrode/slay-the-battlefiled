...
public class card28vfx : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        prefab = Resources.Load<GameObject>("vfx/ripoison");
        ...
        if (gameObject.tag.Contains("ally"))
        {
            ...
            if (opponent != null)
            {
                ...
                if (playerState != null)
                {
                    if (playerState != null)
                    {
                        playerState.GetComponent<PlayerState>().poison += 2;
                    }
                }
                else
                {
    ...
    public IEnumerator FlyAndScalePrefab()
    {
        ...
        while (elapsedTime < duration)
        {
            ...
            float scale = Mathf.PingPong(t * scaleSpeedMultiplier, 0.8f) + 0.5f;  // 더 빠르게 커지고 작아짐
