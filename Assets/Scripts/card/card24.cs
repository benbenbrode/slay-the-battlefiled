...
public class card24 : MonoBehaviour
{
    ...
    public void ActivateEffect(GameObject target)
    {
        ...
        if (target.tag.Contains("opp"))
            me.GetComponent<PlayerState>().poison += 3;
        else
            opp.GetComponent<PlayerState>().poison += 3;

        // Canvas 찾기
        GameObject canvasObject = GameObject.Find("Canvas");

        // 프리팹 로드
        GameObject CardEffectVFX = Resources.Load<GameObject>("vfx/vfx_24");
